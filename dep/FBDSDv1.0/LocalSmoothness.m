function [smoothlocal,qtemp]=LocalSmoothness(x1,y1,x2o,y2o,Neig,mu,lambda,ls,C,Epsilon_Lambda,flag,index)
m=length(x1);
if(nargin<12)
    index=[1:m];
end
...
L=TPSWfromfeatures_FBDSD([x2l,y2l],[x1l,y1l],C,mu,lambda,Epsilon_Lambda);
qtemp=tpswarp([pxl,pyl]',L,C,lambda,Epsilon_Lambda);
smoothlocal(i)=norm(qtemp-[qxl;qyl]);%./norm([maxx-minx;maxy-miny]);    
if(flag==1)
%plot(qtemp(1)+568,qtemp(2),'bo','LineWidth',3);
%plot(qxl+568,qyl,'go','LineWidth',3);
hl=line([qtemp(1)+568,qxl+568],[qtemp(2),qyl])
set(hl,'LineWidth',5,'Color',[0,1,1]);
end
end
